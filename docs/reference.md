...
3. ビルトインの fallback（`default-tapping-term-ms = 200`, `default-quick-tap-ms = 0`, `default-require-prior-idle-ms = 0`, `hold-after-partner-release-ms = 30`）

`hold-after-partner-release-ms` は、plain combo partner が離されてから HT 未確定キーが combo へ転ぶ短い猶予時間（ms）。HOLD 確定を `tapping-term-ms` より早めることはない。partner release が `tapping-term-ms` 境界付近で起きた場合、結果として HOLD 確定が tapping term を少し越えることがある。これは release 順序の揺れを吸収して意図しない modifier 発火を避けるためのものなので、`tapping-term-ms` よりかなり小さく設定する。

HT ごとの HOLD 確定 deadline は次の遅い方になる。
...
複数の HT 未確定キーが同じ plain partner release に関わる場合は、それらの effective deadline の最大値を使う。短い値を持つ HT が、長めに combo release を待ちたい HT の猶予を切り詰めないため。

per-key で上書きできるのは `tapping-term-ms`、`quick-tap-ms`、`require-prior-idle-ms`、`hold-after-partner-release-ms`。
