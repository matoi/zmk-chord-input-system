...
### Raw Keycode Embedding: `CDIS_KEY()`

Some layouts (e.g. naginata) need non-kana keys (backspace, cursor arrows) on certain faces of a NICOLA character key. The `CDIS_KEY()` macro allows embedding a raw ZMK keycode in any kana face slot:

```c
/* Q: unshifted=ヴ, center-shift=Backspace */
CHORD_INPUT_BEHAVIOR5(ng_vu, KANA_VU, CDIS_KEY(BSPC), KANA_NONE, KANA_NONE, KANA_NONE)

/* T: always Left arrow regardless of shift */
CHORD_INPUT_BEHAVIOR5(ng_left, CDIS_KEY(LEFT), CDIS_KEY(LEFT), KANA_NONE, KANA_NONE, KANA_NONE)

/* Modifier-wrapped keycodes also work */
CHORD_INPUT_BEHAVIOR5(ng_del, CDIS_KEY(LS(BSPC)), KANA_NONE, KANA_NONE, KANA_NONE, KANA_NONE)
```

**Mechanism**: The `kana[]` value space is partitioned by `CDIS_RAW_KEYCODE_THRESHOLD` (0x10000):
...
`CDIS_KEY(k)` is defined as `(k)` — a documentation-only wrapper. ZMK keycodes are inherently above the threshold, so the macro serves to signal intent in the DTS source.
    ...
    keymap {
        ...
        nicola_layer {
            ...
            bindings = <
                ...
                CDIS_NICOLA_LM  CDIS_NICOLA_RM
                CDIS_NICOLA_LB  CDIS_NICOLA_RB
                &cdis_on LY_NICOLA  CDIS_NICOLA_LH  CDIS_NICOLA_RH  &cdis_off LY_NICOLA
            >;
        };
    };
};
...
#### レベル 4: かな以外のキーコードの埋め込み（CDIS_KEY）

薙刀式のように、配列内にバックスペースやカーソルキーを含むレイアウトでは、`CDIS_KEY()` で ZMK キーコードを直接かな面に埋め込める:
...
`CDIS_KEY()` はかなと自由に混在でき、同じキーの別の面にかなと生キーコードを同居させられる。詳細は「Kana ID Encoding > Raw Keycode Embedding: CDIS_KEY()」を参照。
...
| `auto-off-keys` で prefix 後に chord-input を抜ける | 実用的。`C-x` や `M-x` の直後を英数側に戻したいときに有効 |
| 「次の1キーをパススルー」behavior | 汎用だが余分な1打鍵が必要 |
| modifier 解放後の短時間パススルー（sticky） | 通常のかな入力直後に誤爆するリスクがあり非推奨 |

**推奨**: まずは modifier-held で完結するショートカットを使う。難しい場合は `auto-off-keys` で prefix 後に chord-input を抜ける運用にするか、必要なコマンドをアプリ側で rebind する。
...
OS → キーボードへの IME 状態通知の仕組みは HID プロトコルに存在しないため、ファームウェア側での自動同期は不可能。ユーザーは `&cdis_off` で手動切り替えを行うか、`auto-off-keys` でズレやすい操作の前後に chord-input を抜けるようにするか、アプリ側の自動 IME 切り替えを無効にする必要がある。

`auto-off-keys` はこの問題そのものを解決する機能ではないが、実運用上は重要な緩和策になる。たとえば `C-x` や `M-x` のように、その後アプリケーション側で英数入力やミニバッファ遷移が起きやすいショートカットを `auto-off-keys` に登録しておけば、prefix の直後に chord-input layer と IME をまとめて OFF に戻せるため、レイヤーと IME の不一致が起きる場面を減らせる。
